...
| VR ladder and swimming locomotion | `game_patch/vr/vr.cpp` | Inside an active climb region or while swimming, forward-stick movement is projected through the tracked HMD pitch so RF receives the vertical component that flat mode normally derives from looking up or down. Three-dimensional movement is normalized after projection. Ordinary ground locomotion remains yaw-only. |
...
| Runtime reference-space recenter continuity | `game_patch/vr/openxr_context.*` | `XR_TYPE_EVENT_DATA_REFERENCE_SPACE_CHANGE_PENDING` for LOCAL space is applied at its specified `changeTime`. AFVR composes the inverse `poseInPreviousSpace` transform with its accumulated application-space offset, switches eyes, hands, layers, menu pointer, and gameplay tracking to a compensated LOCAL reference space, and retains older space handles until shutdown for in-flight frames. Meta/runtime recentering therefore cannot reset HMD-relative yaw back to RF's native body target. |
...
| HMD-directed world interaction | `game_patch/misc/player.cpp`, `vr.cpp` | RF's shared usable-object selector at `0x004897D0` drives both the per-frame interaction overlay and the actual Use keypress. Active local VR temporarily supplies the latest collision-aware HMD world position and orientation to that native selector, then immediately restores the entity eye pose. Physical head rotation and artificial yaw therefore share one target while flat mode and remote entities remain unchanged. |
| VR mounted controls and exit horizon | `game_patch/vr/vr.cpp` | Placed turrets and jeep gunner stations receive HMD-relative yaw/pitch through RF's native `ControlInfo` rotation path. Their stereo view uses a neutral mount frame to avoid applying the flat turret camera rotation twice. Vehicle drivers retain native vehicle-local left-stick movement and receive right-stick yaw/pitch steering; mounted weapon cycling is suppressed to avoid an axis conflict. A same-player attachment transition clears residual pitch/roll from RF's player/eye/automobile controls while preserving yaw, invalidates stale tracked-view caches, and levels the temporary on-foot base until RF's native camera reports stable level frames. This prevents a steeply pitched submarine exit from tilting regular gameplay. |
| VR laser aim assist | `game_patch/vr/vr.cpp`, `vr_render_bridge.*`, `graphics/d3d11/gr_d3d11.*`, `openxr_context.*` | Right-thumbstick click toggles a default-off translucent red beam. Quest-calibrated weapon-local emitter positions are baked into the centralized `VrWeaponCalibration` table and resolve through the final visible one/two-hand weapon transform. One collision result and identical RF-world endpoints are shared by both eyes. Live `vr_muzzle_move`, `vr_muzzle_rotate`, `vr_laser_move`, and `vr_laser_rotate` commands remain available and are included in calibration snapshots. The beam retains its validated 1.8 cm cross-section and explicitly rebound asymmetric eye ViewProj. |
...
| Contextual two-hand gun handling and calibration | `game_patch/vr/vr.cpp`, shared firing/effect consumers | Left squeeze near a per-gun support point acquires a support grip. A stable blend of both aim poses and the physical hand baseline drives the visual weapon, calibrated muzzle, firing, projectiles, flamethrower, and shell orientation. `vr_two_hand_calibrate` temporarily keeps the gun one-handed, consumes a right-trigger confirmation, captures both grip/aim poses, derives and applies a direct weapon-local support point, and logs raw tracking plus projected support measurements for later baking. Throwables and non-guns remain one-handed; left squeeze remains Use when outside a support capture region. |
...
| Native precision/sniper scope presentation | `game_patch/vr/vr.cpp`, `openxr_context.*`, `graphics/d3d11/gr_d3d11.cpp` | The Precision Rifle and Sniper Rifle retain RF's authored zoom FOV, scope mask, reticle, and single native render. While zoom is active that completed image is copied after resolve/gamma into a head-following OpenXR quad using a scope-only state that does not activate menu input or menu recenter behavior. Closing zoom, changing weapon/state, or opening a menu restores stereo world plus HUD layers automatically. |
