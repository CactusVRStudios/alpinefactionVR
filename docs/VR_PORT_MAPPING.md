...
| Room-scale collision and height reset | `game_patch/vr/vr.cpp`, `openxr_context.*` | The HMD center uses RF's low-level swept-sphere world query with a 16 cm radius and invisible collision faces enabled. A single safe center and pushback correction are shared by both stereo eyes and every tracked hand/weapon transform, preventing physical movement from exposing geometry without breaking stereo. Mounted roles are excluded. Both menu-open and menu-close transitions request a new yaw, height, and translation origin; gameplay and menu frame callbacks can each consume the recalibration from their current center pose. |
